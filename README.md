...
3. **Start the DevDocker environment**:
   ...
   This will automatically pull the pre-built image from [Docker Hub](https://hub.docker.com/r/geoguruafrica/devdocker) (`geoguruafrica/devdocker`) if it's not already available locally.
...
The DevDocker image is available on Docker Hub, allowing you to skip the build process and start developing immediately.
...
Source code lives in a Docker named volume for build performance. There are several ways to access it from your host:

**Windows: Direct volume access via WSL2 (Docker Desktop)**

On Windows with Docker Desktop (WSL2 backend), you can browse and edit named volume contents directly from Windows Explorer. Paste this path into the Explorer address bar:

```
\\wsl.localhost\docker-desktop\mnt\docker-desktop-disk\data\docker\volumes\devdocker-workspace\_data
```

This gives you read/write access to the `/workspace` directory inside the container. You can map it as a network drive for convenience (right-click the folder → "Map network drive...").

Other named volumes are accessible the same way — just replace `devdocker-workspace` with the volume name (e.g., `devdocker-home`, `devdocker-maven-repo`).

> **Note**: This path works with recent Docker Desktop versions using the WSL2 backend. Performance is fine for browsing and editing files, but not good for builds, so these should still be run inside the container for optimal speed.
...
Solution: On Windows with Docker Desktop (WSL2 backend), you can access named volume contents directly via Windows Explorer:
```
\\wsl.localhost\docker-desktop\mnt\docker-desktop-disk\data\docker\volumes\devdocker-workspace\_data
```
Alternatively, use:
